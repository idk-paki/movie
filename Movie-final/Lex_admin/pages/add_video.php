  ...
  <aside class="sidenav navbar navbar-vertical navbar-expand-xs border-radius-lg fixed-start ms-2  bg-white my-2" id="sidenav-main">
    ...
    <div class="collapse navbar-collapse  w-auto " id="sidenav-collapse-main">
      <ul class="navbar-nav">
        ...
        <li class="nav-item">
          <a class="nav-link active bg-gradient-dark text-white" href="../pages/add_video.php">
            ...
            <div class="card-body px-0 pb-2">
			  ...
			  <?php/*edit */?>
                 <form name="editfrm" method="post" enctype="multipart/form-data">
					...
					<input  style="border-radius:25px;" type="text" name="video_name" required><br>
					<label style="margin-left:5%;font-size:20px;">Video Price:</label>
					<input  style="border-radius:25px;"type="text" name="video_price" required><br>
					<label style="margin-left:5%;font-size:20px;">Video Date:</label>
					<input  style="border-radius:25px;"type="datetime-local" name="video_date" required><br>
					<label style="margin-left:5%;font-size:20px;">Video Description:</label>
					<br>
					<textarea style="border-radius:25px;margin-left:5%;font-size:20px;" cols="40" rows="4" name="video_details" required></textarea><br>
					...
					<br>
					<br><input style="margin-left:5%;" class="button" type="submit" name="savebtn" value="Create Video"/>
...
if (isset($_POST["savebtn"])) {
    ...
    $temp = $datetime->format('Y-m-d'); // Extract only the date part

    // Check for duplicate video date (only comparing the date part)
    $stmt = $connect->prepare("SELECT COUNT(*) FROM video WHERE DATE(video_date) = ?");
    ...
    if ($count > 0) {
        // Duplicate date found, prevent upload
        echo "<script>
            Swal.fire({
                icon: 'error',
                title: 'Date Conflict',
                text: 'A movie is already scheduled for this date. Please choose another date.',
                confirmButtonText: 'OK'
            }).then((result) => {
                if (result.isConfirmed) {
    ...
    // Insert new video
    $insert_stmt = $connect->prepare("INSERT INTO video(video_name, video_price, video_details, video_img, video_file, video_status, video_date) VALUES (?, ?, ?, ?, ?, ?, ?)");
    $insert_stmt->bind_param("sssssss", $video_name, $video_price, $video_details, $video_img, $video_file, $video_status, $video_date);
    $insert_stmt->execute();
