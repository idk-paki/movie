  ...
  <aside class="sidenav navbar navbar-vertical navbar-expand-xs border-radius-lg fixed-start ms-2  bg-white my-2" id="sidenav-main">
    ...
    <div class="collapse navbar-collapse  w-auto " id="sidenav-collapse-main">
      <ul class="navbar-nav">
        ...
        <li class="nav-item">
          <a class="nav-link text-dark" href="../pages/video_list.php">
		...
		<li class="nav-item">
          <a class="nav-link active bg-gradient-dark text-white" href="../pages/order_manage.php">
            <i class="material-symbols-rounded opacity-5">receipt_long</i>
      ...
      <div class="row">
        <div class="col-12">
          <div class="card my-4">
			...
			<div>
			<input class="border" style="margin-left:10px;" type="text" name="searchname" placeholder="Order ID OR User ID">
...
if (isset($_GET["searchbtn"])) {
  ...
  $search = mysqli_query($connect, "
    SELECT purchased.*, user.user_name 
    FROM purchased 
    JOIN user ON purchased.user_id = user.user_id 
    WHERE purchased.purchased_id LIKE '%$result%' 
    OR user.user_name LIKE '%$result%' 
    ORDER BY purchased.purchased_time DESC
");


    if (!$search) {
        echo "<tbody><tr><td colspan='5' class='align-middle text-center'>SQL Error: " . mysqli_error($connect) . "</td></tr></tbody>";
    } elseif (mysqli_num_rows($search) == 0) {
        echo "<tbody><tr><td colspan='5' class='align-middle text-center'>Result could not be found!</td></tr></tbody>";
    } else {
        while ($row = mysqli_fetch_assoc($search)) {
            $user_id = $row['user_id'];
            $result2 = mysqli_query($connect, "SELECT user_name FROM user WHERE user_id='$user_id'");
            $row2 = ($result2) ? mysqli_fetch_assoc($result2) : null;
?>
            <tbody>
                <tr>
                    <td style="color:black;text-align:center;">
                        <?php echo $row['purchased_id']; ?>
                    </td>
                    <td style="color:black;text-align:center;">
                        <?php echo $row2 ? $row2['user_name'] : "Unknown User"; ?>
                    </td>
                    <td style="color:black;text-align:center;">
                        RM<?php echo $row['total_pay']; ?>
                    </td>
                    <td style="color:black;text-align:center;">
                        <?php echo $row['purchased_time']; ?>
                    </td>
                    <td style="color:black;text-align:center;">
                        <a style="color:black;text-align:center;font-size: 12px;" href="http://localhost/Lex_movie/Lex_admin/pages/makepdf.php?pdf&id=<?php echo $row['user_id']; ?>&purid=<?php echo $row['purchased_id']; ?>">
                            Download
                        </a>
                    </td>
                </tr>
...
<?php
    ...
    $nosearch = mysqli_query($connect, "SELECT * FROM purchased ORDER BY purchased_time DESC");

    if (!$nosearch) {
        echo "<tbody><tr><td colspan='5' class='align-middle text-center'>SQL Error: " . mysqli_error($connect) . "</td></tr></tbody>";
    } else {
        while ($row1 = mysqli_fetch_assoc($nosearch)) {
...
?>
            <tbody>
                <tr>
                    <td style="color:black;text-align:center;">
                        <?php echo $row1['purchased_id']; ?>
                    </td>
                    <td style="color:black;text-align:center;">
                        <?php echo $row2 ? $row2['user_name'] : "Unknown User"; ?>
                    </td>
                    <td style="color:black;text-align:center;">
                        RM<?php echo $row1['total_pay']; ?>
                    </td>
                    <td style="color:black;text-align:center;">
                        <?php echo $row1['purchased_time']; ?>
                    </td>
                    <td style="color:black;text-align:center;">
                        <a style="color:black;text-align:center;font-size: 12px;" href="http://localhost/Lex_movie/Lex_admin/pages/makepdf.php?pdf&id=<?php echo $row1['user_id']; ?>&purid=<?php echo $row1['purchased_id']; ?>">
                            Download
                        </a>
                    </td>
                </tr>
